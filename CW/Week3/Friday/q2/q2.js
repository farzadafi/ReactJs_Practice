...
//A) A function that get name of a team and return list of that team’s players
let teamName = "Tottenham";
let teamPlayer = player.filter(p => p.team === teamName);
console.log(`Tottenham team has ${teamPlayer.length} player`)

//B) A function that get a position and return list of players that play on that position
let positionName = "Striker";
let positionPlayer = player.filter(p => p.position === positionName);
console.log(`Striker position has ${positionPlayer.length} player`)

//C) A function to sort players on their identifier number
player.sort(function (a, b) {
    let keyA = a.identifier,
        keyB = new Date(b.identifier);
...
//D) A function that get a player first name and last name, and check if that player exist on
// our list then return his/her data

let playerName = "Salomon";
if(player.find(p => p.first_name === playerName))
    console.log(`${playerName} is here!`)
else
    console.log(`${playerName} is not here!`)

//E) A function that get some data as argument and add that data to all players’ data

const newPlayer = player.map(addIsActive)
